...
/* Private functions */
static int EthernetConnect(void *context, const char* host, short unsigned port, int timeout_ms)
{
  int ret = 0;
...
static int16_t mqttclient_get_packetid(void)
{
  mPacketIdLast = (mPacketIdLast >= MAX_PACKET_ID) ?
                  1 : mPacketIdLast + 1;
  return (int16_t)mPacketIdLast;
}
...
void loop() {
  ...
  if (rc == 0) {
    ...
    if (enable_lwt) {
      ...
      lwt_msg.total_len = (int16_t)strlen(DEFAULT_CLIENT_ID);
    }
    ...
    if (rc == MQTT_CODE_SUCCESS) {
      ...
      publish.total_len = (int16_t)strlen(TEST_MESSAGE);
